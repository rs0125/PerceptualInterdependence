...
class RGBForensics:
    ...
    def find_traitor(self, signature: np.ndarray, max_users: int = 100) -> int:
        ...
        block maps for each user using AssetBinderComplex, normalizes each expected
        ...
        # Initialize AssetBinderComplex for noise generation
        asset_binder = AssetBinderComplex()
    ...
    def _generate_expected_noise_map(self, shape: Tuple[int, int], user_seed: int, 
                                   asset_binder: AssetBinderComplex) -> np.ndarray:
        """
        Generate expected noise block map for a specific user using AssetBinderComplex logic.
        
        Args:
            shape (Tuple[int, int]): Shape of the noise map (height, width)
            user_seed (int): User seed for noise generation
            asset_binder (AssetBinderComplex): AssetBinderComplex instance for noise generation
            
        Returns:
            np.ndarray: Expected noise map for the user
        """
        # Use AssetBinderComplex's poison map generation method
